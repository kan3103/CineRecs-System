...
class Movie(SQLModel, table=True):
  ...
  total_rating: float = Field(nullable=False, ge=0.0, le=10.0)
  rating_total_count: int = Field(nullable=False, ge=0, default=0)
  runtime: str = Field(nullable=False, min_length=1, max_length=15, regex=r"^\d{1,3} min$")
  release_date: date = Field(nullable=False)
  budget: float = Field(nullable=True, default=0.0, ge=0.0)
  revenue: str = Field(nullable=False, min_length=1, max_length=63, regex=r"^\$\d{1,3}(?:,\d{3})*$")
  description: str = Field(nullable=True, max_length=1022, default='')
  status: bool = Field(nullable=False, default=True)
  poster: str = Field(nullable= True, default = None)
  ...
  genres: list["Genre"] = Relationship(back_populates='genre', link_model=MovieGenre)
